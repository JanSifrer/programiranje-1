...
let rec mapi'''' f list =
  let rec mapi' st acc f list =
    match list with
      | [] -> reverse acc
      | x :: xs -> mapi' (st + 1) (((f x st))::acc) f xs
  in
  mapi' 0 [] f list  
...
let rec unzip_tlrec list =
  let rec unzip_tlrec' acc bcc list = 
    match list with
      | [] -> (reverse(acc), reverse(bcc))
      | (x,y) :: xs -> unzip_tlrec' (x :: acc) (y :: bcc) xs
    in
  unzip_tlrec' [] [] list
